...
with open('D:/.UNIVERSIDAD/AI/data/keywords.json') as json_file:
    # Read the file contents
    keywordsAux = json_file.read()
keywords = re.findall(r'"([^"]+)"', keywordsAux)
print(keywords)
# Eliminar palabras repetidas y ordenar
unique_keywords = list(OrderedDict.fromkeys(keywords))
# Crear diccionario
diccionario = OrderedDict((word, i) for i, word in enumerate(unique_keywords))
...
print(f'Example keywords: {keywords[5]}')
print("\n")
...
tokenizerkeywords =   tf.keras.preprocessing.text.Tokenizer(num_words=MAX_VOCAB_SIZE)
tokenizerkeywords.fit_on_texts(keywords)
sequenceskeywords =   tokenizerkeywords.texts_to_sequences(keywords)
word_index =  tokenizerkeywords.word_index
print(f'Biggest index: {max(max(seq) for seq in sequenceskeywords if len(seq) > 0)}')
print(f'Unique tokens: {len(word_index)}')
print('-' * 20)
print(f'Example comment: {keywords[9]}: {sequenceskeywords[9]}')
print("\n")
...
keywords2 = tf.keras.preprocessing.sequence.pad_sequences(
    sequenceskeywords,
    value=0,
    maxlen=MAX_SEQUENCE_LENGTH
)
...
print(keywords[1])
print(keywords[2])
Y = np.asarray(keywords2).astype(np.float32)
print("-----Y-------")
print(Y)
